...
# Namespace imports for soong_namespace in Android.bp
namespace_imports = [
...
blob_fixups: blob_fixups_user_type = {
    # Script fixes
    'vendor/bin/init.qcom.usb.sh': blob_fixup().regex_replace(
        r'ro.product.marketname',
        'ro.product.odm.marketname'
    ),

    # Seccomp Policy fixes
    'vendor/etc/seccomp_policy/c2audio.vendor.ext-arm64.policy':
        blob_fixup().add_line_if_missing('setsockopt: 1'),
    ...
    'system_ext/lib64/libwfdmmsrc_system.so':
        blob_fixup().add_needed('libgui_shim.so'),
    ...
    ): blob_fixup().add_needed('libcodec2_hidl_shim.so'),

    'vendor/lib64/vendor.libdpmframework.so':
        blob_fixup().add_needed('libhidlbase_shim.so'),
    ...
    ): blob_fixup().replace_needed('libgui.so', 'libgui-xiaomi.so'),

    (
        'system/lib64/libgui-xiaomi.so'
    ): blob_fixup().replace_needed('android.hardware.graphics.common-V5-ndk.so', 'android.hardware.graphics.common-V6-ndk.so'),
    ...
    ): blob_fixup().replace_needed('android.hardware.graphics.allocator-V1-ndk.so', 'android.hardware.graphics.allocator-V2-ndk.so'),

    (
        'vendor/bin/hw/vendor.qti.hardware.display.composer-service',
        'vendor/lib64/libdisplaydebug.so',
        'odm/bin/hw/vendor.xiaomi.sensor.citsensorservice.aidl'
    ): blob_fixup().replace_needed('android.hardware.graphics.composer3-V2-ndk.so', 'android.hardware.graphics.composer3-V3-ndk.so'),
    ...
    ): blob_fixup().clear_symbol_version('AHardwareBuffer_describe'),
